...
export class YahooMarketProvider implements MarketDataProvider {
  ...
  async connect(): Promise<void> {
    this.logger.log('Connected to Yahoo Finance Market Provider (@deprecated Fallback Engine)');
  ...
  private mapInterval(interval: string): string {
    ...
    switch (norm) {
      ...
      case '1h': case '60m': case '60min': return '60m';
      case '1d': case 'day': case 'daily': return '1d';
      case '1wk': case '1w': case 'week': case 'weekly': return '1wk';
      case '1mo': case '1mth': case 'month': case 'monthly': return '1mo';
      default: return '1wk';
    }
  }
  ...
  async getCandles(symbolInput: string, intervalInput: string, range = '1Y'): Promise<CandleData[]> {
    ...
    if (rng === 'MAX') lookbackDays = 7300; // ~20 years
    ...
    if (['1m', '2m', '5m'].includes(interval) && lookbackDays > 7) lookbackDays = 7;
    if (['15m', '30m', '60m'].includes(interval) && lookbackDays > 60) lookbackDays = 60;

    const period1 = new Date(now - lookbackDays * 24 * 60 * 60 * 1000);

    this.logger.debug(`Yahoo Finance chart request (fallback): ${sym} (${interval})`);
    ...
    for (const q of quotes) {
      if (q && q.open !== null && q.high !== null && q.low !== null && q.close !== null) {
    ...
    if (candles.length === 0) {
      throw new Error(`No candles parsed from Yahoo Finance for ${sym} (${interval})`);
    }
  ...
  async getQuote(symbolInput: string): Promise<QuoteData> {
    const sym = this.formatSymbol(symbolInput);
    const yf = yahooFinance as any;
    const quote = (await yf.quote(sym)) as any;
    ...
    return {
      ...
      previousClose: Number((quote.regularMarketPreviousClose || quote.regularMarketPrice).toFixed(2)),
      high: Number((quote.regularMarketDayHigh || quote.regularMarketPrice).toFixed(2)),
      low: Number((quote.regularMarketDayLow || quote.regularMarketPrice).toFixed(2)),
  ...
  private async pollPrices(): Promise<void> {
    ...
    for (const sym of symArray) {
      try {
        ...
        this.logger.debug(`Failed polling fallback price for ${sym}: ${err.message}`);
      }
    }
  }
}
