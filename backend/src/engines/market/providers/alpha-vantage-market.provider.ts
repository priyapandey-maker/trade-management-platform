...
export class AlphaVantageMarketProvider implements MarketDataProvider {
  ...
  async connect(): Promise<void> {
    this.logger.log(
      'Connected to Alpha Vantage Market Provider (Secondary Failover Engine)',
    );
  }
  ...
  async getCandles(
    ...
    if (data['Error Message'] || data['Note'] || !data[timeSeriesKey]) {
      const errMsg =
        data['Error Message'] ||
        (data['Note']
          ? 'Rate limit exceeded (Note returned)'
          : `Missing key ${timeSeriesKey}`);
  ...
  async getQuote(symbolInput: string): Promise<QuoteData> {
    ...
    if (!q || !q['05. price']) {
      throw new Error(
        `Alpha Vantage quote failed for ${sym}: No Global Quote data`,
      );
    }

    const price = Number(parseFloat(q['05. price']).toFixed(2));
    const prevClose = Number(
      parseFloat(q['08. previous close'] || q['05. price']).toFixed(2),
    );
    const change = Number(parseFloat(q['09. change'] || 0).toFixed(2));
    const changePercent = Number(
      parseFloat((q['10. change percent'] || '0').replace('%', '')).toFixed(2),
    );
